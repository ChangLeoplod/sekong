<?php
//购买点数处理
function PayApiBuyFen($fen,$money,$paybz,$orderid,$userid,$username,$ecms_paytype,$payip){
	...
	if($fen)
	{
		//echo "userfen: " . egetmf('userfen');
		//echo "userid:" . egetmf('userid');
...
//商城支付
function PayApiShopPay($ddid,$money,$paybz,$orderid,$userid,$username,$ecms_paytype,$channelid){
	global $empire,$dbtbpre;
	//验证是否重复提交
	$orderid=RepPostVar($orderid);
	$num=$empire->gettotal("select count(*) as total from {$dbtbpre}enewspayrecord where orderid='$orderid' limit 1");
	...
	if($money==$ddr['tmoney'])
	{
		...
		$empire->query("insert into {$dbtbpre}enewspayrecord(id,userid,username,orderid,money,posttime,paybz,type,payip) values(NULL,'$userid','$username','$orderid','$money','$posttime','$paybz','$ecms_paytype','$payip','$channelid');");
	}
	printerror('您已成功购买此订单','../../ShopSys/buycar/',1,0,1);
}
...
//充值类型支付
function PayApiBuyGroupPay($bgid,$money,$orderid,$userid,$username,$groupid,$ecms_paytype,$payip,$channelid){
	...
	//echo "buygroupid level: " . $level_r[$buyr[buygroupid]][level] . "\n";
	//echo "groupid level: " . $level_r[$groupid][level] . "\n";
	//echo "id: " .$buyr['id'];
	//echo "gmoney:" . $buyr['gmoney'];
	if($buyr['id']&&$money==$buyr['gmoney']&&$level_r[$buyr[buygroupid]][level]<=$level_r[$groupid][level])
	{
		//echo "begin update database\n";
		$money=(float)$money;
		//充值
		...
		$empire->query("insert into {$dbtbpre}enewspayrecord(id,userid,username,orderid,money,posttime,paybz,type,payip,channelid) values(NULL,'$userid','$username','$orderid','$money','$posttime','$paybz','$ecms_paytype','$payip','$channelid');");
		//备份充值记录
