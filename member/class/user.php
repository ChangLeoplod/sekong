...
function islogin_shouji($uname=''){
	...
	//{$gotourl=$public_r['newsurl']."e/member/login/";}
	$petype=1;
	$cr=$empire->fetch1("select ".eReturnSelectMemberF('userid,username,email,groupid,userfen,money,userdate,zgroupid,havemsg,checked,registertime')." from ".eReturnMemberTable()." where ".egetmf('username')."='$username' limit 1");
	if(!$cr['userid'])
	{
		EmptyEcmsCookie();
		if(!getcvar('returnurl'))
		...
		printerror_shouji("NotSingleLogin","",$petype);
	}
	if($cr['checked']==0)
	{
		EmptyEcmsCookie();
		...
		printerror_shouji("NotCheckedUser",'',$petype);
	}
	//默认会员组
	if(empty($cr['groupid']))
	{
...
//查询用户已查阅过的付费图集
function ReturnUserViewRecord($userid)
{
	global $empire,$dbtbpre;
	//手机用户已经查阅过的付费图集
        //是否有历史记录
        $tempArray=array();
        $viewrec_sql=$empire->query("select id,classid,truetime from {$dbtbpre}enewsdownrecord where userid='$userid' and online=2");
        while($viewrec=$empire->fetch($viewrec_sql))
        {
                $temp=array(
                        'id'=>$viewrec['id'],
                        'classid'=>$viewrec['classid'],
                        'truetime'=>$viewrec['truetime']
                );
                $tempArray[]=$temp;
        }
	$re[bakr]=$tempArray;
