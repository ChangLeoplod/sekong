...
//错误提示：手机
function printerror_shouji($error="",$gotourl="",$ecms=0,$noautourl=0,$novar=0){
	global $empire,$editor;
	echo "editor: " . $editor;
	echo "ecms: " . $ecms;
	if($editor==1){$a="../";}
	elseif($editor==2){$a="../../";}
...
function page1($num,$line,$page_line,$start,$page,$search){
	...
	for($i=$starti;$i<$totalpage&&$no<$page_line;$i++)
	{
		...
		if($page==$i)
		{
			$is_1="<a class=\"active\" href=\"javascript:void(0)\">";
			$is_2="</a>";
		}
		else
		{
...
function _yzm(){
	global $public_r;
	if(strstr($_SERVER['HTTP_HOST'],$public_r['add_www_92game_net_domaind'])!=false){
		$domain='http://'.$public_r['add_www_92game_net_domaind'];
	}else{
		$domain = '';
	}
	...
	$str=md5(md5($str));
	//echo($str);
	if($str!=$public_r['add_www_92game_net_yzm'])
		exit('');
}
//返回替换列表
...
//格式化信息目录
function FormatPath_shouji($classid,$mynewspath,$enews=0){
        global $class_r;
	echo " in FormatPath_shouji \n";
        if($enews)
        {
                $newspath=$mynewspath;
        }
        ...
        $path_shouji=ECMS_PATH."shouji/".ReturnSaveInfoPath($classid,$id);
        $returnpath="";
        $r=explode("/",$newspath);
        $count=count($r);
        for($i=0;$i<$count;$i++){
                ...
                $createpath_shouji=$path_shouji.$returnpath;
                $mk_shouji=DoMkdir($createpath_shouji);
                if(empty($mk_shouji))
                {
                        printerror("CreatePathFail","");
                }
