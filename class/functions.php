...
function GetFpicToTpic($classid,$id,$num=1,$getfirsttitlespic=0,$swidth=0,$sheight=0,$fstb=1){
	...
	if($picr['fileid'])
	{
		...
		if($getfirsttitlespic==1&&swidth)//缩略图
		{
...
function NewsBq($classid,$indextext,$enews=0,$doing=0){
	...
	//by jlzhang 2015-04-16
	//测试刷新首页
	if($enews == 1) //生成手机首页文件
	{
		//执行代码
		$shouji_string = GenMainPageForMobile();
		$shoujitruefile = ECMS_PATH.'shouji/'.str_replace('.html', '.php', GetFileName($truefile));
...
function ListHtml($classid,$fields,$enews=0,$userlistr=""){
	...
	if($enews==0)//子栏目列表
	{
		...
		$dopath=ECMS_PATH.$doclasspath."/";
		$dopath_shouji=ECMS_PATH."shouji/".$doclasspath."/";
		echo "enews==0 dopath_shouji: " . $dopath_shouji . "\n";
	...
	elseif($enews==5)//标题分类列表
	{
		...
		$dopath=ECMS_PATH.$doclasspath."/";
                ...
                $dopath_shouji=ECMS_PATH."shouji/".$doclasspath."/";
                echo "enews==5 dopath_shouji: " . $dopath_shouji . "\n";
	...
	elseif($enews==3)//大栏目列表
	{
		...
		$dopath=ECMS_PATH.$doclasspath."/";
                ...
                $dopath_shouji=ECMS_PATH."shouji/".$doclasspath."/";
                echo "enews==3 dopath_shouji: " . $dopath_shouji . "\n";
	...
	while($k=$empire->fetch($sql))
	{
		...
		//for mobile
		preg_match_all('/<a href=(.*) class=(.*)>[\s]*<img src=(.*) alt=(.*)>/',$repvar,$result);
		//$href_string = str_replace("html", "php", $result[1][0]);
		//这里很tricky,主要是加入shouji这个目录
		//$href_string = preg_replace("/\"/", "\"/shouji", $href_string, 1);
		//$class_string = $result[2][0];
		$alt_string = $result[4][0];
		//$src_string = $result[3][0];
		$href_string="/shouji".str_replace("html","php",$k['titleurl']);
		...
		if($no%$lencord==0||($num%$lencord<>0&&$num==$no))
		{
			...
			$retstring = "\$arr = array();
				...
				\$arrListInfo = array();
				";
			$retstring = $retstring . $arrayElement. "
					\$arr['ListInfo'] = \$arrListInfo;
					\$strt = json_encode(\$arr, JSON_UNESCAPED_UNICODE);
					echo(\$strt);
			";
			$retstring = AddPHPHead(). $retstring. AddPHPTail();
			WriteFiletext($path_shouji,$retstring);
			$retstring='';
			$arrayElement='';
			$string='';
		}
...
function GetHtml($classid,$id,$add,$ecms=0,$doall=0){
	...
	$doclasspath_shouji=ECMS_PATH."shouji/".$iclasspath;
	$createinfopath=$doclasspath;
	$createinfopath_shouji=$doclasspath_shouji;
	//建立日期目录
	$newspath='';
	$newspath_shouji='';
	if($add[newspath])
	{
		...
		//for mobile
		$createpath_shouji=$doclasspath_shouji.$add[newspath];
		if(!file_exists($createpath_shouji))
		{
			$r[newspath_shouji]=FormatPath_shouji($add[classid], $add[newspath],1);
		}
		$createinofPath_shouji.=$add[newspath].'/';
		$newspath_shouji=$add[newspath].'/';
		//end
	}
	//新建存放目录
	if($class_r[$add[classid]][filename]==3)
	{
		...
		echo "createinfopath: " . $createinfopath;
		DoMkdir($createinfopath);
		//for mobile
		$createinfopath_shouji.=ReturnInfoSPath($add['filename']);
		DoMkdir($createinfopath_shouji);
		$fn3=1;
	}
	//存文本
	...
	//为手机写入内容页
	$file_shouji=$doclasspath_shouji.$newspath_shouji.$add[filename].'.php';
	$shouji_string = GenContextPageForMobile();
	//$addlevel=str_replace("../../","../../../",$addlevel);
	//$addlevel=str_replace("CheckLevel.php","CheckLevel_Shouji.php",$addlevel);
	WriteFileText($file_shouji, AddPHPHead().$shouji_string.AddPHPTail());
}
...
//为手机生成内容页php文件
function GenContextPageForMobile()
{
	...
	"
	\/\/搜集组图
	\$arrListInfo_pics = array();
	";
	//这是以前比较tricky的做法
	/*
	preg_match_all('/<[img|IMG|iframe|IFRAME].*?src=(.*?(?:[\.gif|\.jpg|\.png|\.mp4])).*?[\/]?>/',$ecms_gr[newstext],$result);
	echo "why";
	for($i=0;$i<count($result[0]);$i++)
	{
		$result_string = $result[0][$i];
		preg_match_all('/src=(.*[\.jpg|\.gif|\.png|\.mp4])/',$result_string, $result_temp);
		$final_array=explode(" ", $result_temp[1][0]);
		...
		";
		$retstring = $retstring .
		"\$arrListInfo_pics[] = \$arrTemp;

		";
	}
	*/
	$content_test=stripslashes($navinfor['newstext']);
        $img_arr=get_imgs($content_test);
        for($i =0; $i< count($img_arr);$i++)
        {
		$retstring=$retstring .
